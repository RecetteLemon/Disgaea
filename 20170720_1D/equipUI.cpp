...
void equipUI::update()
{
	...
	{
		if (!_equipOn) _equipOn = true;
		_equip->setCharNum(_kind);
	}
	if(KEYMANAGER->isOnceKeyDown('L')) _equipOn = false;
...
void equipUI::render()
{
	if (!_equipOn) showstatUI();
	else if (_equipOn) _equip->render();
}

void equipUI::showstatUI()
{
	...
	DIRECT2D->drawTextD2D(DIRECT2D->createBrush(RGB(0, 0, 0), 1), L"°íµñ", 30, str, 535, 95 + 2 + 2, 580, 100 + 2 + 2);
	swprintf_s(str, L"%d", STATMANAGER->getPlayerStat()[2]._lv);
	DIRECT2D->drawTextD2D(DIRECT2D->createBrush(RGB(0, 0, 0), 1), L"°íµñ", 30, str, 535, 145 + 2 + 2 + 2, 580, 150 + 2 + 2 + 2);
	swprintf_s(str, L"%d", STATMANAGER->getPlayerStat()[3]._lv);
	DIRECT2D->drawTextD2D(DIRECT2D->createBrush(RGB(0, 0, 0), 1), L"°íµñ", 30, str, 535, 195 + 2 + 2 + 2 + 2, 580, 200 + 2 + 2 + 2 + 2);
	swprintf_s(str, L"%d", STATMANAGER->getPlayerStat()[4]._lv);
	DIRECT2D->drawTextD2D(DIRECT2D->createBrush(RGB(0, 0, 0), 1), L"°íµñ", 30, str, 535, 245 + 2 + 2 + 2 + 2 + 2, 580, 250 + 2 + 2 + 2 + 2 + 2);


	swprintf_s(str, L"Adell");
	DIRECT2D->drawTextD2D(DIRECT2D->createBrush(RGB(0, 0, 0), 1), L"°íµñ", 28, str, _rc[0].right + 100, 50 + 2, _rc[0].right + 300, 50 + 2);
	swprintf_s(str, L"Rozalin");
	DIRECT2D->drawTextD2D(DIRECT2D->createBrush(RGB(0, 0, 0), 1), L"°íµñ", 28, str, _rc[1].right + 100, 100 + 2 + 2, _rc[1].right + 300, 100 + 2 + 2);
	swprintf_s(str, L"Pram");
	DIRECT2D->drawTextD2D(DIRECT2D->createBrush(RGB(0, 0, 0), 1), L"°íµñ", 28, str, _rc[2].right + 100, 150 + 2 + 2 + 2, _rc[2].right + 300, 150 + 2 + 2 + 2);
	swprintf_s(str, L"Valvatorez");
	DIRECT2D->drawTextD2D(DIRECT2D->createBrush(RGB(0, 0, 0), 1), L"°íµñ", 28, str, _rc[3].right + 100, 200 + 2 + 2 + 2 + 2, _rc[3].right + 300, 200 + 2 + 2 + 2 + 2);
	swprintf_s(str, L"Cleric");
