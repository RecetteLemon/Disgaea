...
HRESULT inven::init()
{
	...
	i_cursor = IMAGEMANAGER->findImage(L"cursor");
	i_bar = IMAGEMANAGER->findImage(L"moveBar");
...
void inven::render()
{
	IMAGEMANAGER->findImage(L"itemBag")->render(_itemBag.left, _itemBag.top);
	IMAGEMANAGER->findImage(L"sort")->frameRender(_sort.left, _sort.top, 0, _currentFrameY,false, 1.0f);
	...
	if(_upDirectionOn) IMAGEMANAGER->frameRender(L"upDirection", _upDirection.left, _upDirection.top, 0, 0,false, _alpha);
	if(_downDirectionOn) IMAGEMANAGER->frameRender(L"downDirection", _downDirection.left, _downDirection.top, 0, 0, false,_alpha);
	if (_isInfoOn) IMAGEMANAGER->findImage(L"infoBox")->render(_infoBox.left, _infoBox.top);
}
...
void inven::infoBox()
{
	...
	for (int i = 0; i < 24; i++)
	{
		...
		{
			if (i_slot[i] != IMAGEMANAGER->findImage(L"none")) _isInfoOn = true;
			else if (i_slot[i] == IMAGEMANAGER->findImage(L"none")) _isInfoOn = false;
		}
	}
}
