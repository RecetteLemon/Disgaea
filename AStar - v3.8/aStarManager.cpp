...
void aStarManager::pathFinder(aStarTile* currentTile)
{
	...
	if (!addOpenList(currentTile).size())
	{
		//_vMoveList.clear();
		//vectorClear();
		return;
...
void aStarManager::render()
{
	...
	for (int i = 0; i < _vMoveList.size(); ++i)
	{
		...
		DIRECT2D->drawTextD2D(DIRECT2D->_defaultBrush, str, _vMoveList[i].centerX - CAMERAMANAGER->getX(), 
			_vMoveList[i].centerY - CAMERAMANAGER->getY(),
			_vMoveList[i].centerX + 25 - CAMERAMANAGER->getX()
			, _vMoveList[i].centerY + 25 - CAMERAMANAGER->getY());
	}
	wsprintf(str, L"%d", _vMoveList.size());
	DIRECT2D->drawTextD2D(DIRECT2D->_defaultBrush, str, WINSIZEX / 2, 0, WINSIZEX / 2 + 50, 10);
...
void aStarManager::moveListUpdate()
{
	_vMoveList.clear();
}