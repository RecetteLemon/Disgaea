...
void aStar::render()
{
	...
	for (int i = 0; i < _vMoveList.size(); i++)
	{
		...
		DIRECT2D->drawTextD2D(DIRECT2D->_defaultBrush, str, _vMoveList[i].centerX - +CAMERAMANAGER->getX(), _vMoveList[i].centerY - +CAMERAMANAGER->getY(), _vMoveList[i].centerX + 30 - +CAMERAMANAGER->getX(), _vMoveList[i].centerY + 30 - +CAMERAMANAGER->getY());
	}
}
